...
def create_background_image(mode, size):
    ...
    if mode == 2:
        if not os.path.isdir(BG_IMAGE_DIR):
            print(f"警告：找不到背景圖資料夾 '{BG_IMAGE_DIR}'。將使用純黑背景。")
            return create_background_image(0, size)
        logo_files = [f for f in os.listdir(BG_IMAGE_DIR) if f.startswith('logo') and f.endswith(('.png', '.jpg', '.jpeg'))]
        if not logo_files:
            print(f"警告：在 '{BG_IMAGE_DIR}' 中找不到 logo 圖檔。將使用純黑背景。")
            return create_background_image(0, size)
        chosen_logo_path = os.path.join(BG_IMAGE_DIR, random.choice(logo_files))
        try:
            logo_img = Image.open(chosen_logo_path)
        except IOError:
            print(f"警告：無法開啟圖片 '{chosen_logo_path}'。將使用純黑背景。")
            return create_background_image(0, size)
        bg_img = Image.new('RGB', size, color=(0, 0, 0))
        logo_img.thumbnail((size[0] * 0.8, size[1] * 0.8), Image.Resampling.LANCZOS)
...
def get_font(ball_diameter, font_size_ratio):
    ...
    try:
        ...
        print("警告：找不到字體 'arial.ttf'，將使用預設字體。")
...
def draw_single_ball(canvas_img, draw_obj, char_info, font, is_flashed=False, color_mode="normal", font_mode=0, showtype=0):
    ...
    if font_mode == 0:
        ...
        deformed_img = char_img.resize((new_w, new_h), resample=Image.Resampling.BICUBIC)
        
        angle = random.randint(*ROTATION_ANGLE_RANGE)
        final_char_img = deformed_img.rotate(angle, resample=Image.Resampling.BICUBIC, expand=True)
...
def generate_animated_char_array_gif(rc_mode, password_str, num_distractors_input, font_mode, background_mode, flash_mode, showtype):
    ...
    # PIL 的 save 函數
    frames[0].save(output_filename, save_all=True, append_images=frames[1:], duration=durations, loop=0, optimize=True)
    print(f"\nGIF 動畫生成完成！ -> {output_filename}")
    shutil.rmtree(TEMP_FRAMES_DIR, ignore_errors=True) # 清理臨時資料夾

def main():
    # --- 參數數量檢查 ---
    if len(sys.argv) != 8:
        print("用法: python <腳本名> <陣列模式> <密碼> <干擾數> <字體模式> <背景模式> <閃動模式> <顯示模式>")
        print("陣列模式: 3 (3x3), 4 (4x4), 5 (5x5), 6 (6x6)")
        print("字體模式: 0(常規), 1(變形旋轉), 2(變形旋轉+線條+雜訊塊)")
        print("背景模式: 0(純黑), 1(雜訊亮點), 2(隨機Logo圖)")
        print("閃動模式: 0(單點依序), 1(接龍依序)")
        print("顯示模式: 0(規則圓球陣列), 1(隨機重疊圓球(限制遮蔽率))") # 更新說明
        sys.exit(1)
    ...
    try:
        ...
        print(f"錯誤：參數無效。請檢查您的輸入。({e})")
        sys.exit(1)

    generate_animated_char_array_gif(rc_mode, password, distractor_count, font_mode, background_mode, flash_mode, showtype)
